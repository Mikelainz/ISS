%==============================================
% WorldTheory.pl for actor qatestplans
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatuqatestplans).	%%old version (deprecated)
actorobj(qatuqatestplans).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor qatestplans
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of qatestplans" ). 
...
%-------------------------------------------------
%  User static rules about qatestplans
%------------------------------------------------- 
/*
------------------------------------------------------------------------
...
actorOp( Op )   :- actorobj(Actor),
				   output( actorOppppppppppppp( Actor, Op  ) ),
				   java_catch(
...
setActorOpResult( Op, Res ):- %% Res is $obj_xxx	
	...
	output( actorOpDone( Op,ResStr  ) ),  
   	storeActorOpResult( Op, ResStr ).
storeActorOpResult( Op, R ):-
	output( storeActorOpResult( Op,R  ) ),  
...
executeCmd( Actor,  printCurrentEvent(ARG), Events, Plans, done(printCurrentEvent) ):-
	...
	%% actorPrintln( printCurrentEvent( ARGS ) ).
%%% ---------  addRule	---------------	

...
class QaRobotActor extends it.unibo.qactor.robot.RobotActor{
	public QaRobotActor(
		...
		super(name, ctx, "./srcMore/it/unibo/voyager/plans.txt", worldTheoryPath,
		outEnvView, it.unibo.qactor.robot.RobotSysKb.setRobotBase(ctx, baserobot) , defaultPlan);
	}
}

public class AbstractVoyager extends QaRobotActor { 
...
protected IActorAction  action;
	...
	public AbstractVoyager(String actorId, QActorContext myCtx, IOutputEnvView outEnvView ,String baserobot)  throws Exception{
		super(actorId, myCtx,  
		"./srcMore/it/unibo/voyager/WorldTheory.pl",
		setTheEnv( outEnvView ) ,baserobot , "initi");		
		this.planFilePath = "./srcMore/it/unibo/voyager/plans.txt";
	...
	* ------------------------------------------------------------
	*/
    public boolean initi() throws Exception{	//public to allow reflection
    try{
    	int nPlanIter = 0;
    	//curPlanInExec =  "initi";
    	boolean returnValue = suspendWork;		//MARCHH2017
    while(true){
    	curPlanInExec =  "initi";	//within while since it can be lost by switchlan
    	nPlanIter++;
    		temporaryStr = "\"voyager STARTS\"";
    		println( temporaryStr );  
    		if( ! planUtils.switchToPlan("doWork").getGoon() ) break;
    		temporaryStr = "\"voyager ENDS\"";
    		println( temporaryStr );  
    break;
    }//while
    return returnValue;
    }catch(Exception e){
       //println( getName() + " plan=initi WARNING:" + e.getMessage() );
       QActorContext.terminateQActorSystem(this); 
       return false;  
    }
    }
    ...
    try{
    	...
    	boolean returnValue = suspendWork;		//MARCHH2017
    while(true){
    	curPlanInExec =  "doWork";	//within while since it can be lost by switchlan
    	nPlanIter++;
    		temporaryStr = "\"voyager doWork\"";
    		println( temporaryStr );  
    		//forward
    		//if( ! execRobotMove("doWork","forward",70,0,3000, "" , "") ) break;
    		    aar = execRobotMove("doWork","forward",70,0,3000, "" , "");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "doWork";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    		//left
    		//if( ! execRobotMove("doWork","left",0,0,500, "" , "") ) break;
    		    aar = execRobotMove("doWork","left",0,0,500, "" , "");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "doWork";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    		//forward
    		//if( ! execRobotMove("doWork","forward",70,0,2000, "" , "") ) break;
    		    aar = execRobotMove("doWork","forward",70,0,2000, "" , "");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "doWork";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    		//right
    		//if( ! execRobotMove("doWork","right",0,0,500, "" , "") ) break;
    		    aar = execRobotMove("doWork","right",0,0,500, "" , "");
    		    if( aar.getInterrupted() ){
    		    	curPlanInExec   = "doWork";
    		    	if( ! aar.getGoon() ) break;
    		    } 			
    ...
    */
    protected void initSensorSystem(){		
    	try {
    		String goal = "consult( \"./src/it/unibo/voyager/sensorTheory.pl\" )";
    ...
    */
    protected void addSensorObservers(){
    	for (ISensor<?> sensor : Configurator.getInstance().getSensors()) {
    		//println( "voyager sensor= "  + sensor.getDefStringRep() );
    		//println( "voyager sensor class= "  + sensor.getClass().getName() );

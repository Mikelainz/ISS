...
public abstract class AbstractMyled extends QActor { 
	...
	protected IActorAction  action;
		...
		*/
	    	...
	    	nPlanIter++;
	    		...
	    		if( currentEvent.getEventId().equals("buttonEvent") ){
	    		 		...
	    		 		parg =  updateVars(  Term.createTerm("startBlinkLed(BLINKINTERVAL)"), Term.createTerm("startBlinkLed(BLINKINTERVAL)"), 
	    		 			...
	    		 			if( parg != null ){
	    		 				 if( ! planUtils.switchToPlan("startBlink").getGoon() ) break; 
	    		...
	    		if( currentEvent.getEventId().equals("buttonEvent") ){
	    		 		...
	    		 		parg =  updateVars(  Term.createTerm("startBlinkLed(BLINKINTERVAL)"), Term.createTerm("stopBlinkLed"), 
	    		 			    		  					Term.createTerm(currentEvent.getMsg()), parg);
	    		 			if( parg != null ){
	    		 				 if( ! planUtils.switchToPlan("stopBlink").getGoon() ) break; 
	    		 			}//else println("guard  fails");  //parg is null when there is no guard (onEvent)
	    		 }
	    ...
	    public boolean startBlink() throws Exception{	//public to allow reflection
	    try{
	    	int nPlanIter = 0;
	    	//curPlanInExec =  "startBlink";
	    	boolean returnValue = suspendWork;		//MARCHH2017
	    while(true){
	    	curPlanInExec =  "startBlink";	//within while since it can be lost by switchlan
	    	nPlanIter++;
	    		...
	    		//QActorUtils.solveGoal(parg,pengine );
	    		//senseEvent
	    		aar = planUtils.senseEvents( 1000,"buttonEvent","continue",
	    		"" , "",ActionExecMode.synch );
	    		if( ! aar.getGoon() || aar.getTimeRemained() <= 0 ){
	    			//println("			WARNING: sense timeout");
	    			addRule("tout(senseevent,"+getName()+")");
	    ...
	    return returnValue;
	       ...
	       //println( getName() + " plan=startBlink WARNING:" + e.getMessage() );
	       QActorContext.terminateQActorSystem(this); 
	       return false;  
	    }
	    }
	    public boolean stopBlink() throws Exception{	//public to allow reflection
	    try{
	    	int nPlanIter = 0;
	    	//curPlanInExec =  "stopBlink";
	    	boolean returnValue = suspendWork;		//MARCHH2017
	    while(true){
	    	curPlanInExec =  "stopBlink";	//within while since it can be lost by switchlan
	    	nPlanIter++;
	    ...
	    return returnValue;
	       ...
	       //println( getName() + " plan=stopBlink WARNING:" + e.getMessage() );
	       QActorContext.terminateQActorSystem(this); 
	       return false;  
	    }
	    }
